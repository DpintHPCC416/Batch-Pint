    ...
    def initFlowTable(self):
        for i in range(0, 4): # pod i
            for j in range(0, 2): # subnet j
                ...
                cur_switch.flow_tables['DpintIngress.tbl_forward'] = []
                for k in range(0, 2):
                    entry = FlowEntry()
                    ...
                    entry.action = "DpintIngress.forward"
                    egress_port_id = getPortId(switch_port, switch)
                    entry.action_params = {'dstAddr' : host.mac, 'port' : egress_port_id}
                    cur_switch.flow_tables['DpintIngress.tbl_forward'].append(entry)

                    cur_switch.flow_tables['MyEgress.tbl_check_last_hop']
